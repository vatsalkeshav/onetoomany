...
impl Invaders {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.finished() {
            ...
            if downwards {
                let new_duration = max(self.move_timer.duration().as_millis() - 250, 250); // go downwards faster everytime
                self.move_timer
                    .tick(Duration::from_millis(new_duration as u64));
    ...
    pub fn kill_invader_at(&mut self, x: usize, y: usize) -> bool {
        if let Some(idx) = self
            .army
            .iter()
            .position(|invader: &Invader| (invader.x == x) && (invader.y == y))
        {
